...
void test('removes a closed mobile drawer from the accessibility tree and tab order', () => {
  const { container } = render(
    <WorkspaceNavigation
      collapsed={false}
      inactive
...
void test('exposes a modal drawer close control without adding a navigation item', async () => {
  ...
  assert.ok(screen.getByRole('dialog', { name: 'Workspace navigation' }));
  const navigation = screen.getByRole('navigation', { name: 'Primary navigation' });
  assert.deepEqual(
    within(navigation)
      .getAllByRole('link')
...
void test('keeps collapsed actions named and provides tooltips for their hidden labels', async () => {
  ...
  assert.ok(overviewLink.querySelector('.workspace-navigation__action-label[hidden]'));
  assert.ok(signOutButton.querySelector('.workspace-navigation__action-label[hidden]'));
